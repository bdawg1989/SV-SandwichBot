...
namespace SysBot.Base
{
    public static class EchoUtil
    {
        ...
        public static readonly List<Action<byte[], string, EmbedBuilder>> SandwichForwarders = new();
        ...
        public static void SandwichEmbed(byte[] bytes, string fileName, EmbedBuilder embeds)
        {
            foreach (var fwd in SandwichForwarders)
            {
                try
                {
                ...
                {
                    LogUtil.LogInfo($"Exception: {ex} occurred while trying to echo: SandwichEmbed to the forwarder: {fwd}", "Echo");
                    LogUtil.LogSafe(ex, "Echo");
                }
            }
        }
