...
namespace SysBot.Pokemon.Discord
{
    ...
    public class EchoModule : ModuleBase<SocketCommandContext>
    {
        ...
        private class EchoChannel
        {
            ...
            public readonly Action<byte[], string, EmbedBuilder> SandwichAction;
            public string EmbedResult = string.Empty;

            public EchoChannel(ulong channelId, string channelName, Action<string> action, Action<byte[], string, EmbedBuilder> SandwichAction)
            {
                ChannelID = channelId;
                ChannelName = channelName;
                Action = action;
                SandwichAction = SandwichAction;
            }
        }
            ...
            var thumbnailOptions = new List<string>
    {
        ...
        [Alias("aec")]
        [Summary("Makes the bot post Sandwich embeds to the channel.")]
        [RequireSudo]
        public async Task AddEchoAsync()
        {
            ...
            await ReplyAsync("Added Sandwich Embed output to this channel!").ConfigureAwait(false);
        }
        ...
        private static async Task<bool> SandwichEmbedAsync(ISocketMessageChannel c, byte[] bytes, string fileName, EmbedBuilder embed, int maxRetries = 2)
        {
            int retryCount = 0;
            while (retryCount < maxRetries)
            {
        ...
        private static void AddEchoChannel(ISocketMessageChannel c, ulong cid)
        {
            ...
            Action<byte[], string, EmbedBuilder> rb = async (bytes, fileName, embed) => await SandwichEmbedAsync(c, bytes, fileName, embed).ConfigureAwait(false);

            EchoUtil.Forwarders.Add(l);
            EchoUtil.SandwichForwarders.Add(rb);
        ...
        public async Task ClearEchosAsync()
        {
            ...
            EchoUtil.SandwichForwarders.Remove(echo.SandwichAction);
        ...
        public async Task ClearEchosAllAsync()
        {
            ...
            EchoUtil.SandwichForwarders.RemoveAll(y => Channels.Select(x => x.Value.SandwichAction).Contains(y));
            Channels.Clear();
